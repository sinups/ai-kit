...
function renderEssentials(): string {
  return [
    ...
    "- `toolRenderers` on `AgentChat`, `MessageList` and `ToolRenderer` adds or replaces cards. Keys are full part types, `tool-<Name>`, such as `tool-Deploy` or `tool-mcp__git__search`; a bare name only matches `mcp__user-tools__<name>`. Renderers receive `CustomToolRendererProps`: `name`, `input`, `output`, `result`, `structuredContent`, `status`, `toolCallId`, `part`, `onAction`. `output` keeps its 0.3 value (for an MCP tool the text of its content, parsed when it holds JSON); `result` is the MCP `CallToolResult` and the recommended input. `onAction` reports to `onToolAction`.",
    ...
    "- Transcript layout: `presentation` on `AgentChat` and `MessageList` is `'cards'` by default; `rowsPresentation` gives the flat rows of a terminal client, `quietPresentation` muted lines for MCP calls and calls described in `toolCatalog`, formatted through `toolOutputs` and `toolArgs`, and one folded line per turn (`Thought · used 2 tools · 26s`). Import them from the package root and pass the value.",
    "- Tool context: `toolCatalog` takes MCP tool definitions keyed by `mcp__<server>__<tool>` (`title`, `description`, `annotations`, `inputSchema`, `outputSchema`); results are read strictly by the MCP `CallToolResult` (`structuredContent` by `outputSchema`, else the content blocks by type, text never parsed as JSON); `toolArgs` and `toolOutputs` format arguments and results, keyed like `toolRenderers`, `tool-mcp__<server>__*` allowed, `null` keeps the kit summary; a result formatter gets `output` with its 0.3 value and `result`, the `CallToolResult`, which is recommended; `locale` formats numbers and dates.",
...
function renderSkills(): string {
  return page("/docs/skills", "Skills", [
