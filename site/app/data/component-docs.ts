...
/>`,
      },
      {
        ...
        title: 'Presentation',
        content:
          "`presentation` sets how the transcript lays out tool calls. `'cards'`, the default, keeps a card per call. `rowsPresentation` shows the flat transcript of a terminal client: a marker, the call and its answer under a gutter, with even spacing. `quietPresentation` turns MCP calls, thinking and any call `toolCatalog` describes (a local tool of the host included) into muted lines that open into their arguments and result; the line reads its outcome through `toolOutputs` and its opened arguments through `toolArgs` when a formatter returns a string, a failure included, and falls back to the kit summary on `null`; the thinking and the finished or running calls a turn makes before its answer fold into one line such as `Thought · used 2 tools · 26s`, and a call that waits for a decision in `approvals` stays outside that line with a frame and its buttons. Other tool parts keep their cards. Both presentations are values imported from the package root, so a host that does not import them does not ship their code.",
      },
      {
        type: 'example',
...
export function Example() {
  return <MessageList messages={messages} status="ready" />;
      ...
      },
      {
        ...
        content:
          "`partRenderers` renders part types the list does not know, keyed by `part.type`; each renderer gets `part`, `messageId`, `index` and `chatStatus`, and a part without one stays hidden. `text` and `reasoning` can be overridden as well: `partRenderers.text` replaces the Markdown of an answer, and `reasoning`, hidden by default, shows through its renderer; planning, the working row and copying still read the `text` parts. Create the object once, outside the component or with `useMemo`: a new object on every render re-renders finished messages, and the list warns about it in development. `sendScroll=\"prompt-top\"` puts a sent question at the top and grows the answer under it; `sendScroll=\"prompt-top-hold\"` also keeps the list still while the answer grows and follows it again only after the user scrolls to the bottom (wheel, touch, keyboard, scrollbar) or presses the new messages button, which shows once the answer reaches below the viewport; `streamingCaret` shows a caret after the growing text, and `lazyTurns` (a turn count, `true` means 50) skips layout and paint of finished turns out of view with `content-visibility`. All three are off by default in `MessageList` and `AgentChat`. The list is a `log` with `aria-busy` while an answer streams; `labels.answerReady`, `labels.answerFailed` or, after `stopped`, `labels.answerStopped` is announced once when it ends; `AgentChat` sets `stopped` from its stop button. `messageActions.actions` adds host buttons to the toolbar of each message, usually `MessageActionButton`.",
      },
      {
        type: 'example',
        title: 'Presentations',
