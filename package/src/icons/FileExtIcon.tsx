...
/** Brand icon for well-known source file extensions (ts, js, json). Renders nothing for other files. */
export function FileExtIcon({ filename, size = 10, className, style }: FileExtIconProps) {
  const ext = filename.split('.').pop()?.toLowerCase() ?? '';
  const common = { width: size, height: size, className, style: { flexShrink: 0, ...style } };
  ...
  if (ext === 'js' || ext === 'mjs' || ext === 'cjs' || ext === 'jsx') {
    return (
      <svg viewBox="0 0 32 32" {...common}>
        <rect x="2" y="2" width="28" height="28" rx="1.312" fill="var(--mantine-color-yellow-3)" />
        <path
